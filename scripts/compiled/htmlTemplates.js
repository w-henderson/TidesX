...
(function (HTML) {
    ...
    function createTideTime(info) {
        ...
        var height = UserPreferences.settings.imperialMeasurements ? info.height * 3.28084 : info.height;
        var heightString = height.toFixed(2) + (UserPreferences.settings.imperialMeasurements ? "ft" : "m");
        timeSpan.textContent = "".concat(info.time, " (").concat(heightString, ")");
    ...
    function updateExtraInfo(element, tides) {
        ...
        tides.forEach(function (tide) {
            ...
            if (tide.Height !== undefined && timeObj !== undefined) { // basically if not a low tide in an estuary
                var height = UserPreferences.settings.imperialMeasurements ? tide.Height * 3.28084 : tide.Height;
                var heightString = height.toFixed(2) + (UserPreferences.settings.imperialMeasurements ? "ft" : "m");
                infoSpan.textContent = "".concat(timeObj.getHours().toString().padStart(2, "0"), ":").concat(timeObj.getMinutes().toString().padStart(2, "0"), " (").concat(heightString, ")");
