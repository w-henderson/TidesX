...
function initFavouritesPage() {
    ...
    if (favouriteLocations.length == 0)
        HTMLRefs.tabs.home.innerHTML = "Your favourite locations will appear here once you've chosen some.";
    else
        HTMLRefs.tabs.home.innerHTML = "";
...
function requestFavouriteLocationAsync(favourite, stationId) {
    ...
    API.getTides(stationId).then(function (tideTimes) {
        ...
        var tide1Date = new Date(Date.parse(futureTideTimes[0].DateTime + "Z"));
        var tide2Date = new Date(Date.parse(futureTideTimes[1].DateTime + "Z"));
...
function heightInterpolation(tides) {
    ...
    for (var i = 0; i < tides.length; i++) {
        ...
        if (new Date(Date.parse(nextTide.DateTime + "Z")).getTime() > timeNow.getTime())
            break;
    }
    var nextTideTime = new Date(Date.parse(nextTide.DateTime + "Z")).getTime();
